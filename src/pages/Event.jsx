...
const Event = () => {
  ...
  useEffect(() => {
    if (eventData && eventData.fights.length > 0) {
      const allFightsCompleted = eventData.fights.every((fight) =>
        completedFights.has(fight.id)
      );

      setEventCompleted(allFightsCompleted);
      const stringedId = eventData.id.toString();

      // Ensure eventData.id exists before updating status
      if (
        allFightsCompleted &&
        ...
        const updateStatus = async () => {
          try {
            await updateEventStatus(stringedId, "completed");
            console.log(`Event ${eventData.id} marked as completed`);
          } catch (error) {
  ...
  }, [completedFights, eventData]); // Include eventData in the dependencies
  ...
  return (
    ...
    >
      <h1>{eventCompleted ? "Event Completed!" : "Event In Progress"}</h1>
