...
const Rankings = () => {
  ...
  const getWeightClassFighters = () => {
    ...
    return [
      ...rankedPositions.map(
        (fighter, index) => fighter || { vacant: true, displayRank: index + 1 }
      ),
      ...unrankedFighters,
    ];
  };
  ...
  const handleConfirmRankingSlots = async () => {
    try {
      ...
      const fightersToUpdate = Array.from(updatedFighters.values()).filter(
        (updatedFighter) => {
          const originalFighter = fighters.find(
            (f) => f.personid === updatedFighter.personid
          );
  ...
  const handleUpdateRanking = async () => {
    ...
    try {
      // Get current fighters in weight class
      const weightClassFighters = fighters.filter(
        (f) => f.weightClass === selectedChampionship.weightClass
      );

      const newRank =
        selectedRank === "" || selectedRank === "null"
          ? null
          : parseInt(selectedRank);
      ...
      const updatedFighters = normalizeRankings([
        ...
        ...weightClassFighters.filter(
          (f) => f.personid !== selectedFighter.personid
        ),
      ]);
      ...
      const fightersToUpdate = completeUpdatedFighters.filter(
        (updatedFighter) => {
          const originalFighter = fighters.find(
            (f) => f.personid === updatedFighter.personid
          );
  ...
  return (
    <Container maxWidth="lg" sx={{ mt: 4, mb: 4 }}>
        ...
        <Card>
          <CardContent>
            <Box sx={{ mb: 3, display: "flex", alignItems: "center", gap: 2 }}>
              <Typography variant="h5">{selectedChampionship.name}</Typography>
              ...
              <Table>
                ...
                <TableBody>
                  ...
                  {getWeightClassFighters()
                    .filter(
                      (f) =>
                        f.personid === selectedChampionship.currentChampionId
                    )
                    .map((fighter) => (
                      <TableRow
                  ...
                  {getWeightClassFighters()
                    ...
                    .map((fighter) => {
                      ...
                      const isRanked =
                        fighter.ranking && fighter.ranking <= maxRankings;
          ...
          <Typography>
            {getAffectedFighters().length === 1
              ? `Are you sure you want to do this? ${
                  getAffectedFighters()[0].firstname
                } ${getAffectedFighters()[0].lastname} will become unranked.`
              : `Are you sure you want to do this? ${
                  getAffectedFighters().length
                } fighters will become unranked:`}
          </Typography>
          {getAffectedFighters().length > 1 && (
            <List dense>
      ...
      <Dialog open={openDialog} onClose={() => setOpenDialog(false)}>
        <DialogTitle>
          Update Ranking for {selectedFighter?.firstname}{" "}
          {selectedFighter?.lastname}
        </DialogTitle>
        <DialogContent>
          <FormControl fullWidth sx={{ mt: 2 }}>
            ...
            >
              {[...Array(maxRankings)].map((_, i) => {
                ...
                const currentFighter = fighters.find(
                  (f) =>
                    f.weightClass === selectedChampionship.weightClass &&
                ...
                const isCurrentFighterRank =
                  selectedFighter?.ranking === rankNumber;
                ...
                return (
                  <MenuItem
                    ...
                    sx={{
                      ...
                      fontWeight:
                        currentFighter || isCurrentFighterRank
                          ? "normal"
                          : "light",
                      color: isCurrentFighterRank
                        ? "primary.main"
                        : currentFighter
                        ? "text.primary"
                        : "text.secondary",
