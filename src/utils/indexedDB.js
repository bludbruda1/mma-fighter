...
export const updateChampionshipWithHistory = async (
  championship,
  historyEntry
) => {
  try {
...
export const updateChampionshipWithResult = async (
  championship,
  fightResult,
  isDefense = false
) => {
  try {
    ...
    if (isDefense) {
      ...
      const updatedReigns = championship.reigns.map((reign) => {
        if (
          reign.championId === championship.currentChampionId &&
          !reign.endDate
        ) {
          return {
            ...reign,
...
export const isChampionshipDefense = (fight, championships) => {
  ...
  return (
    fight.fighter1.personid === championship.currentChampionId ||
